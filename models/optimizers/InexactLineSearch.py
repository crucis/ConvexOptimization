...
class InexactLineSearch(optimizer):
    ...
    def _line_search(self):
        ...
        self.grad_f_L = gk * self.direction_vector if not hasattr(gk, '__iter__') else np.transpose(gk) * self.direction_vector
        
        # step 3
        if np.abs(self.grad_f_L) > self.xtol:
            self.a0 = -2*delta_f0 / self.grad_f_L
        else:
            self.a0 = 1
        ...
        for _ in range(self.maxIter):
            ...
            # step 5
            if self.f0 > self.f_L + self.rho*(self.a0 - self.alpha_L) * self.grad_f_L:
            ...
            if not entered_step5:
                ...
                self.grad_f0 = gk * self.direction_vector if not hasattr(gk, '__iter__') else np.transpose(gk) * self.direction_vector
                # step 7
                if self.grad_f0 < self.sigma * self.grad_f_L:
                    self.delta_a0 = ((self.a0 - self.alpha_L) * self.grad_f0 ) / (self.grad_f_L - self.grad_f0)
