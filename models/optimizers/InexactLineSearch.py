...
class InexactLineSearch(optimizer):
    ...
    def _compute_a0tilde(self):
        numerator = (self.a0 - self.alpha_L)**2 * self.grad_f_L
        denominator = 2*(self.f_L - self.f0 + (self.a0 - self.alpha_L) * self.grad_f_L)
    ...
    def _line_search(self):
        ...
        print(self.x_k.shape)
        print(self.direction_vector.shape)
        ...
        # step 3
        if np.linalg.norm(self.grad_f_L) > self.xtol:
            self.a0 = np.linalg.norm(-2*delta_f0 / self.grad_f_L)
        else:
            self.a0 = 1
        ...
        for _ in range(self.maxIter):
            ...
            # step 5
            if np.linalg.norm(self.f0) > np.linalg.norm(self.f_L + self.rho*(self.a0 - self.alpha_L) * self.grad_f_L):
                if self.a0 < self.alpha_U:
                    self.alpha_U = self.a0
                self.a0tilde = np.linalg.norm(self._compute_a0tilde())
            ...
            if not entered_step5:
                ...
                # step 7
                if np.linalg.norm(self.grad_f0) < np.linalg.norm(self.sigma * self.grad_f_L):
                    self.delta_a0 = ((self.a0 - self.alpha_L) * np.linalg.norm(self.grad_f0) ) / np.linalg.norm(self.grad_f_L - self.grad_f0)
